/**
 * Ingestia dziennych konwersji Google Ads wg akcji konwersji.
 * POST /api/ingest/ads-conversions
 * Auth: X-Ingest-Secret (shared secret z env INGEST_SECRET)
 *
 * Payload: { shop_id: string, rows: ConversionRow[] }
 * conversions_value_micros — jak Google Ads API (1 zł = 1 000 000 micros).
 */
...
interface ConversionRow {
  ...
  conversions: number;             // REAL — Ads zwraca wartości ułamkowe
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  let body: ConversionsPayload;
  try {
    body = (await request.json()) as ConversionsPayload;
  ...
  if (rows.length > 0) {
    await db.batch(
      rows.map((r) =>
        db
          .prepare(
            `INSERT INTO fact_ads_conversions_daily
               (shop_id, date, conversion_action, conversions, conversions_value)
             VALUES (?, ?, ?, ?, ?)
             ON CONFLICT (shop_id, date, conversion_action) DO UPDATE SET
